...
public abstract class Message
{
    ...
    public RelativeLayout getBubbleView(ChatAdapter.ViewHolder viewHolder)
    {
        ...
        if (message.isSelf())
        {
            ...
            ImageLoader.getInstance().displayImage(Urls.getImgUrl(ConfigManager.instance().getUserPic()), viewHolder.rightAvatar);
            return viewHolder.rightMessage;
        }
        else
        {
            ImageLoader.getInstance().displayImage(getUserFace(), viewHolder.leftAvatar);
